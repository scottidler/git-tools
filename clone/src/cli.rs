...
#[command(
    after_help = "Layout: `clone <org>/<repo>` always produces a flat checkout (no bare/worktree layout). For a bare-container + worktrees layout, use `worktree init <org>/<repo>` instead; `worktree migrate`/`worktree flatten` convert an existing checkout in place. Shell integration: `clone shell-init zsh` prints a cd-wrapper function; install it with `eval \"$(command clone shell-init zsh)\"` in your .zshrc so clone cd's you into the new checkout."
)]
pub struct Cli {
