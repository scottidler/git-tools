// clone — `--flatten`: collapse a gratuitous bare container back into a flat
...
/// `clone --flatten` works from a linked worktree, a subdirectory, or the
...
fn container_from_dir(dir: &Path) -> Result<PathBuf> {
    ...
    if !out.status.success() {
        bail!(
            "not inside a git repository - run --flatten from within the bare container you want to collapse, or pass org/repo"
        );
    }
...
/// `clone.cfg` `[clone] default`, used only if the container advertises no
