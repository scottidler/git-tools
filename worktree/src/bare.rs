// worktree — bare-container helpers: container resolution plus the bare
// acquisition primitives relocated from `clone` (the clone/worktree split).
//
...
//
// The shared read/mutate primitives (`is_bare_container`, `default_branch`,
// `ref_exists`, `add_worktree`) live in `common::bare` so `clone` and `worktree`
// can't drift. This module adds the worktree-tool-specific container resolution
// and the fresh-acquisition setup (`setup_bare_container`/`reconcile_container`).
...
/// Scalar acquisition inputs for a bare-container setup/reconcile. This replaces
/// the clone-shaped `Config` the code depended on before the split: `worktree`'s
/// own `Config` has none of these fields, so the relocated setup takes this
/// purpose-built struct instead (closes the split's "adapt, not copy" finding).
///
/// `default_branch` is included even though the design's field list named only
/// the five transport inputs: `ensure_default_worktree` needs the fallback
/// branch, so carrying it here keeps the acquisition self-contained rather than
/// threading a sixth positional argument through every call.
#[derive(Debug, Clone)]
pub struct AcquireArgs {
    /// Root the container is created under: `<clonepath>/<org>/<repo>`.
    pub clonepath: PathBuf,
    /// Primary (SSH) remote base tried first; HTTPS is the fallback.
    pub remote: String,
    /// Optional reference mirror for a fast clone.
    pub mirrorpath: Option<PathBuf>,
    /// Per-org SSH key driving `GIT_SSH_COMMAND` for the clone attempt.
    pub ssh_key: Option<PathBuf>,
    /// Verbose transport logging.
    pub verbose: bool,
    /// Last-resort default branch, used only when the remote advertises none.
...
pub fn setup_bare_container(args: &AcquireArgs, spec: &RepoSpec) -> Result<PathBuf> {
    let repospec = spec.to_string();
    let container = args.clonepath.join(&repospec);
    ...
    ensure_default_worktree(args, &container)
}
...
pub fn reconcile_container(args: &AcquireArgs, container: &Path) -> Result<PathBuf> {
...
fn ensure_default_worktree(args: &AcquireArgs, container: &Path) -> Result<PathBuf> {
    ...
    let branch = default_branch(container, args.default_branch.as_deref())?;
